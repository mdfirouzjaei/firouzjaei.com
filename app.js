...
const FOCUSED_TREE_MIN_X_GAP = 184;
...
function isStartingPerson(person) {
  if (ancestryLinkIds(person).length) return false;
  const spouseHasAncestry = (person.spouseIds || []).some((spouseId) => ancestryLinkIds(personById(spouseId)).length);
  return !spouseHasAncestry;
}

function startingPeople() {
  const roots = state.people.filter(isStartingPerson);
  return sortTreePeople(roots.length ? roots : state.people.filter((person) => !ancestryLinkIds(person).length));
}
...
function rootAncestorIds(personId, visited = new Set()) {
  ...
  if (!person) return [];
  const upstreamIds = ancestryLinkIds(person);
  if (!upstreamIds.length) return [person.id];
  const roots = upstreamIds.flatMap((upstreamId) => rootAncestorIds(upstreamId, new Set(visited)));
...
function personHasAncestor(personId, ancestorId, visited = new Set()) {
  ...
  visited.add(personId);
  const person = personById(personId);
  return ancestryLinkIds(person).some((upstreamId) => personHasAncestor(upstreamId, ancestorId, visited));
}
...
function pathFromRootToPerson(rootId, personId, visited = new Set()) {
  ...
  const person = personById(personId);
  if (!person) return [];
  for (const upstreamId of ancestryLinkIds(person)) {
    const upstreamPath = pathFromRootToPerson(rootId, upstreamId, new Set(visited));
    if (upstreamPath.length) return [...upstreamPath, personId];
  }
  return [];
}
...
function descendantIdsOf(personId, visited = new Set()) {
  if (!personId || visited.has(personId)) return [];
  visited.add(personId);
  return treeChildPeopleOf(personId).flatMap((child) => [child.id, ...descendantIdsOf(child.id, visited)]);
}

function expandablePersonIds() {
  return state.people.filter((person) => treeChildPeopleOf(person.id).length).map((person) => person.id);
}
...
function descendantCount(personId, visited = new Set()) {
  ...
  visited.add(personId);
  return treeChildPeopleOf(personId).reduce((total, child) => total + 1 + descendantCount(child.id, visited), 0);
}
...
function navigableChildPeopleOf(personId) {
  const children = treeChildPeopleOf(personId);
...
function searchContextPeople(searchTerm = "") {
  ...
  matches.forEach((person) => {
    ancestryLinkIds(person).forEach((upstreamId) => visibleIds.add(upstreamId));
    (person.spouseIds || []).forEach((spouseId) => visibleIds.add(spouseId));
    treeChildPeopleOf(person.id).forEach((child) => {
      visibleIds.add(child.id);
      ancestryLinkIds(child).forEach((upstreamId) => visibleIds.add(upstreamId));
    });
  });
...
function visibleTreePeople(searchTerm = "") {
  ...
  while (changed) {
    ...
    Array.from(visibleIds).forEach((id) => {
      ...
      const children = navigationRootId ? navigableChildPeopleOf(id) : treeChildPeopleOf(id);
...
function treeDepthMap() {
  ...
  let changed = true;
  let passes = 0;
  const maxPasses = Math.max(1, state.people.length * 2);
  while (changed && passes < maxPasses) {
    passes += 1;
    changed = false;
    state.people.forEach((person) => {
      const upstreamDepths = ancestryLinkIds(person).map((upstreamId) => depths.get(upstreamId)).filter((depth) => depth !== undefined);
      if (upstreamDepths.length) {
        const nextDepth = Math.max(Math.min(...upstreamDepths) + 1, Number(person.generation || 0));
...
function rowGroupAnchor(group) {
  ...
  const descendants = group.filter((person) => ancestryLinkIds(person).length);
...
function orderTreeRowGroupMembers(group) {
  const groupIds = new Set(group.map((person) => person.id));
  const defaultOrder = group
    .slice()
    .sort(
      (a, b) =>
        genderSortValue(a) - genderSortValue(b) ||
  ...
  const hub = group
    .slice()
    .sort((a, b) => {
      const aSpouses = (a.spouseIds || []).filter((id) => groupIds.has(id)).length;
      const bSpouses = (b.spouseIds || []).filter((id) => groupIds.has(id)).length;
      return bSpouses - aSpouses || defaultOrder.indexOf(a) - defaultOrder.indexOf(b);
    })[0];
  const spouses = (hub?.spouseIds || []).map(personById).filter((person) => person && groupIds.has(person.id));
  if (!hub || spouses.length < 2) return defaultOrder;

  const left = [];
  const right = [];
  spouses
    .slice()
    .sort((a, b) => Number(a.slot || 0) - Number(b.slot || 0) || a.name.localeCompare(b.name, "fa"))
    .forEach((spouse, index) => (index % 2 === 0 ? left : right).push(spouse));
  const connectedIds = new Set([hub.id, ...spouses.map((spouse) => spouse.id)]);
  return [...left.reverse(), hub, ...right, ...defaultOrder.filter((person) => !connectedIds.has(person.id))];
}
...
function orderTreeRowPeople(generationPeople, positionedAncestors = new Map()) {
  ...
  sorted.forEach((person) => {
    ...
    while (queue.length) {
      ...
      (current.spouseIds || [])
        .map(personById)
        .filter((spouse) => spouse && rowIds.has(spouse.id) && !placed.has(spouse.id))
  ...
  orderedGroups.forEach((group, index) => {
    ...
    const upstreamIds = ancestryLinkIds(anchor).slice().sort();
    const key = upstreamIds.length ? upstreamIds.join(":") : `unconnected:${index}`;
    let bucket = bucketByParents.get(key);
    if (!bucket) {
      const parentXs = upstreamIds.map((upstreamId) => positionedAncestors.get(upstreamId)?.x).filter(Number.isFinite);
      bucket = {
        groups: [],
        firstIndex: index,
...
function branchBridgeRootForPerson(person) {
  ...
  if (!ancestryLinkIds(person).length) return "";
...
function drawFocusedFamilyRelationships(svg, people, positions, focusId) {
  currentTreeRelationshipBounds = null;
  const roles = focusedFamilyRoleMap(focusId, people);
  const visibleIds = new Set(people.map((person) => person.id));
  ...
  people.forEach((person) => {
    (person.spouseIds || []).forEach((spouseId) => {
      ...
      const spouse = personById(spouseId);
      const personPosition = positions.get(person.id);
      const spousePosition = positions.get(spouseId);
      if (!spouse || !personPosition || !spousePosition) return;
      drawnMarriages.add(key);

      const left = personPosition.x <= spousePosition.x ? personPosition : spousePosition;
      const right = personPosition.x <= spousePosition.x ? spousePosition : personPosition;
      const marriageY = (personPosition.y + spousePosition.y) / 2 + 6;
      const startX = left.x + TREE_NODE_HALF_WIDTH;
      const endX = right.x - TREE_NODE_HALF_WIDTH;
      const title = `پیوند همسری ${person.name} و ${spouse.name}`;
      addLine(svg, startX, marriageY, endX, marriageY, "marriage-line focused-marriage-line", title);
      addCircle(svg, (startX + endX) / 2, marriageY, "marriage-dot focused-marriage-dot");
    });
  });

  const parentIds = new Set(
    people
      .filter((person) => ["father", "mother", "parent"].includes(roles.get(person.id)))
      .map((person) => person.id)
  );
  const childGroups = new Map();
  people
    .filter((person) => roles.get(person.id) === "child")
    .forEach((child) => {
      let visibleParentIds = (child.parentIds || []).filter((parentId) => parentIds.has(parentId));
      if (!visibleParentIds.length && parentIds.has(focusId)) visibleParentIds = [focusId];
      const key = visibleParentIds.slice().sort().join(":") || `single:${focusId}`;
      const group = childGroups.get(key) || { parentIds: visibleParentIds, children: [] };
      group.children.push(child);
      childGroups.set(key, group);
    });

  const groups = Array.from(childGroups.values()).sort((left, right) => {
    const leftX = Math.min(...left.children.map((child) => positions.get(child.id)?.x ?? 0));
    const rightX = Math.min(...right.children.map((child) => positions.get(child.id)?.x ?? 0));
    return leftX - rightX;
  });
  const childPositions = groups.flatMap((group) => group.children.map((child) => positions.get(child.id)).filter(Boolean));
  if (!childPositions.length) return;
  const branchY = Math.min(...childPositions.map((position) => position.y)) - TREE_NODE_HALF_HEIGHT - 34;
  const usesSeparateFamilyBranches = groups.length > 1;
  const focusPerson = personById(focusId);
  const focusPosition = positions.get(focusId);

  groups.forEach((group) => {
    const positionedParents = group.parentIds
      .map((parentId) => ({ id: parentId, position: positions.get(parentId) }))
      .filter((parent) => parent.position);
    const positionedChildren = group.children
      .map((child) => ({ child, position: positions.get(child.id) }))
      .filter((child) => child.position)
      .sort((left, right) => left.position.x - right.position.x);
    if (!positionedChildren.length) return;

    let anchorX = positionedParents[0]?.position.x ?? focusPosition?.x ?? positionedChildren[0].position.x;
    let anchorY = (positionedParents[0]?.position.y ?? NODE_BASE_Y) + TREE_NODE_HALF_HEIGHT;
    if (focusPosition && positionedParents.length > 1 && group.parentIds.includes(focusId)) {
      const partner = positionedParents.find((parent) => parent.id !== focusId);
      if (partner) {
        const direction = Math.sign(partner.position.x - focusPosition.x) || 1;
        const sameSideSpouses = (focusPerson?.spouseIds || [])
          .map((spouseId) => ({ id: spouseId, position: positions.get(spouseId) }))
          .filter(
            (spouse) =>
              spouse.position && Math.sign(spouse.position.x - focusPosition.x) === direction
          )
          .sort(
            (left, right) =>
              Math.abs(left.position.x - focusPosition.x) - Math.abs(right.position.x - focusPosition.x)
          );
        const sideIndex = Math.max(0, sameSideSpouses.findIndex((spouse) => spouse.id === partner.id));
        const availableGap = Math.max(12, currentTreeNodeXGap - TREE_NODE_HALF_WIDTH * 2);
        anchorX =
          focusPosition.x +
          direction *
            (TREE_NODE_HALF_WIDTH + (availableGap * (sideIndex + 1)) / (sameSideSpouses.length + 1));
        anchorY = focusPosition.y + 6;
      }
    } else if (positionedParents.length > 1) {
      anchorX =
        positionedParents.reduce((sum, parent) => sum + parent.position.x, 0) /
        positionedParents.length;
      anchorY = positionedParents[0].position.y + 6;
    }

    const childXs = positionedChildren.map((child) => child.position.x);
    const familyTitle = `فرزندان ${positionedParents
      .map((parent) => personById(parent.id)?.name)
      .filter(Boolean)
      .join(" و ")}`;
    if (usesSeparateFamilyBranches) {
      const partner = positionedParents.find((parent) => parent.id !== focusId);
      const stemX = partner?.position.x ?? anchorX;
      const stemY = partner ? partner.position.y + TREE_NODE_HALF_HEIGHT : anchorY;
      ...
      addLine(
        ...
        childCenterX,
        branchY,
        "descent-line focused-family-connector focused-direct-connector",
        familyTitle
      );
      addCircle(svg, childCenterX, branchY, "descent-junction focused-descent-junction", 4);
      if (childXs.length > 1) {
        addLine(
          svg,
          Math.min(...childXs),
          ...
          Math.max(...childXs),
          branchY,
          "descent-line focused-family-connector",
          familyTitle
        );
      }
      positionedChildren.forEach(({ child, position }) => {
        addLine(
          svg,
          position.x,
          branchY,
          position.x,
          position.y - TREE_NODE_HALF_HEIGHT,
          "descent-line focused-family-connector",
          `${child.name}، ${familyTitle}`
        );
      });
      return;
    }

    addLine(svg, anchorX, anchorY, anchorX, branchY, "descent-line focused-family-connector", familyTitle);
    addCircle(svg, anchorX, branchY, "descent-junction focused-descent-junction", 4);
    addLine(
      svg,
      Math.min(anchorX, ...childXs),
      branchY,
      Math.max(anchorX, ...childXs),
      branchY,
      "descent-line focused-family-connector",
      familyTitle
    );
...
function drawRelationships(svg, people, positions) {
  const searchTerm = $("#familySearch")?.value.trim() || "";
  if (focusedSubtreeId && !normalizeSearchText(searchTerm)) {
    drawFocusedFamilyRelationships(svg, people, positions, focusedSubtreeId);
    return;
  }
  currentTreeRelationshipBounds = null;
  const drawnMarriages = new Set();
  ...
  people.forEach((person) => {
    person.spouseIds?.forEach((spouseId) => {
      ...
      // Multiple spouses use a lower shelf so each couple has a distinct path and child branch.
      if (multiSpouseHub) {
        ...
        const usesSpouseShelf =
          hubSpouses.length >= 2 && hubSpouses.every((item) => positions.get(item.id).y > hubPosition.y);
...
function nextRootSlot() {
  const rootSlots = state.people
    .filter((person) => Number(person.generation || 0) === 0 && !ancestryLinkIds(person).length)
    .map((person) => Number(person.slot || 0));
